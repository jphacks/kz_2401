...
### 特長・注力したこと（こだわり等）
#### 1. ハードウェアの活用
* ソフトウェアのみでは解決できないことに注目。
* 外部とのケーブルの接続は電源のみとした。
* コンパクトで大きな面積をとらないタテ型を採用した。
* 配線を綺麗にした。
* モーターの不安定な信号の制御のため外部電源を採用した。

#### 2. データベースとの連携
SQlite3を用いて、ダッシュボードとデータベースを連携した。
...
### 解決出来ること
#### 現状の課題点
* 現代人の悩みとして、すぐにスマホを触ってしまい集中力が続かない現状がある。
* スマホを触ることを禁止するアプリもあるが、スマホの中にあるためスマホ禁止起動終了前後に
スマホをさわってしまいあっというまに時間がたっているということがある。
...
#### 4.顔認識が作動
USBカメラによる顔認識が作動し、独自の数理モデルに基づいて学習中の姿勢や集中度を測定します。
...
### 独自技術
#### ハッカソンで開発した独自機能・技術
* 集中度スコアリングアルゴリズム - 独自の数理モデルに基づいて算出されている。学習中の姿勢や集中度を自動的に評価し、リアルタイムで集中力を可視化します。
